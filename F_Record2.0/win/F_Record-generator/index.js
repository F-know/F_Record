...
(function () {
    ...
    // 定义我们的扩展id和菜单栏显示名称
    var plugin_id = "F_Record 2.0";
    var menu_name = "F_Record 2.0";
    ...
    async function handleImageChanged(document) {
    ...
    async function refreshImage() {
        ...
        try {
            ...
            if(abc){
                ...
                if (pixmap) {
                    // 保存文件到本地，获取到该图片的路径
                    const output = await saveImageToJPG(pixmap, document.id);
                    console.log(3);
                    // 将该路径发送给插件面板
                    const file = require("path").join(__dirname, 'event-dispatch.jsx');
     ...
     * @param documentId
     */
    async function saveImageToJPG(pixmap, documentId) {
